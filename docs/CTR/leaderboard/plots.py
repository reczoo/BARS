...
def show_table_gauc(csv_path):
    df = pd.read_csv(csv_path)
    df["Model"] = df.apply(lambda x: f"<a href={x['Paper URL']}>{x['Model']}</a>", axis=1)
    del df["Paper URL"]
    df["Running Steps"] = df["Running Steps"].map(lambda x: f"<a href={x}>🔗</a>")
    df = df.sort_values(by=["gAUC"], ascending=False).reset_index(drop=True)
    df.insert(0, "Rank", range(1, len(df) + 1))
    df[['gAUC', 'AUC', 'Logloss']] = df[['gAUC', 'AUC', 'Logloss']].applymap('{:.4f}'.format)
    show(df, lengthMenu=[10, 20, 50, 100], classes="display")

def show_plot_gauc(csv_path):
    df = pd.read_csv(csv_path).sort_values(by="gAUC", ascending=True)
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(
        go.Scatter(x=df["Model"], y=df["gAUC"], name="gAUC", mode='lines+markers',
                   line=dict(color="#3e8c58", shape="spline", smoothing=1.3), marker=dict(size=7)),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(x=df["Model"], y=df["AUC"], name="AUC", mode='lines+markers',
    ...
    fig.update_layout(
        title="Sorted benchmarking results by gAUC",
        title_x=0.5,
        plot_bgcolor='white',
        autosize=True,
        width=890,
