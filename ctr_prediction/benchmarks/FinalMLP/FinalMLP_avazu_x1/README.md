...
3. Both `dataset_config.yaml` and `model_config.yaml` files are available in [FinalMLP_avazu_x1_tuner_config_03](./FinalMLP_avazu_x1_tuner_config_03). Make sure that the data paths in `dataset_config.yaml` are correctly set.

4. Run the following script to start training and evaluation.

    ```bash
    cd FuxiCTR/model_zoo/FinalMLP
    nohup python run_expid.py --config XXX/benchmarks/FinalMLP/FinalMLP_avazu_x1_tuner_config_03 --expid FinalMLP_avazu_x1_018_488deefd --gpu 0 > run.log &
    tail -f run.log
    ```
