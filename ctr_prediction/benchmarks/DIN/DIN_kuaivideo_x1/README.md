...
3. Both `dataset_config.yaml` and `model_config.yaml` files are available in [DIN_kuaivideo_x1_tuner_config_02](./DIN_kuaivideo_x1_tuner_config_02). Make sure that the data paths in `dataset_config.yaml` are correctly set.

4. Run the following script to start training and evaluation.

    ```bash
    cd FuxiCTR/model_zoo/DIN
    nohup python run_expid.py --config XXX/benchmarks/DIN/DIN_kuaivideo_x1_tuner_config_02 --expid DIN_kuaivideo_x1_013_fc4bf206 --gpu 0 > run.log &
    tail -f run.log
    ```
