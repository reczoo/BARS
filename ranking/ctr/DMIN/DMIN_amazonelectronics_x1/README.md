## DMIN_amazonelectronics_x1

A hands-on guide to run the DMIN model on the AmazonElectronics_x1 dataset.
...
3. Both `dataset_config.yaml` and `model_config.yaml` files are available in [DMIN_amazonelectronics_x1_tuner_config_05](./DMIN_amazonelectronics_x1_tuner_config_05). Please make sure that the data paths in `dataset_config.yaml` are correctly set.

4. Run the following script to start training and evaluation.

    ```bash
    cd FuxiCTR/model_zoo/DMIN
    nohup python run_expid.py --config YOUR_PATH/DMIN/DMIN_amazonelectronics_x1_tuner_config_05 --expid DMIN_amazonelectronics_x1_007_2401549e --gpu 0 > run.log &
    tail -f run.log
    ```
