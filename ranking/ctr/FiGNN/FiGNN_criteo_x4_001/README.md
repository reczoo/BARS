...
3. Both `dataset_config.yaml` and `model_config.yaml` files are available in [FiGNN_criteo_x4_tuner_config_01](./FiGNN_criteo_x4_tuner_config_01). Make sure the data paths in `dataset_config.yaml` are correctly set to what we create in the last step.

4. Run the following script to start.

    ```bash
    cd FiGNN_criteo_x4_001
    nohup python run_expid.py --config ./FiGNN_criteo_x4_tuner_config_01 --expid FiGNN_criteo_x4_006_097e8622 --gpu 0 > run.log &
    tail -f run.log
    ```
